...
class PlotterCallback(tf.keras.callbacks.Callback):
    def __init__(self, logdir, X_train, X_test, y_train, y_test, model):
        ...
        self.y_test = y_test
        self.model = model
        self.y_test_str = [str(x) for x in self.y_test]
    ...
    def on_epoch_end(self, epoch, logs):
        ...
        with self.file_writer.as_default():
            ...
            data = tf.data.Dataset.from_tensor_slices(self.X_test).batch(128)
            X_test_pca = PCA(2).fit_transform(
                tf.concat(
                    [
                        self.model.call_latent(d)
            ...
            plot = sns.scatterplot(
                x=X_test_pca[:, 0], y=X_test_pca[:, 1], hue=self.y_test_str
            ).get_figure()
            tf.summary.image(
                f"generated_data/latent",
                self.plot_to_image(plot),
                step=epoch,
            )
