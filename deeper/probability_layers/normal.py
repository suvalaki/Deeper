...
class RandomNormalEncoder(Layer, Scope):
    ...
    def call(self, inputs, training=False, outputs=None):
        ...
        if outputs is not None:
            ...
            sample = mu + r_norm * tf.math.sqrt(tf.exp(logvar)+self.epsilon)
...
class RandomStandardNormalEncoder(Layer, Scope):
    def __init__(
        self, 
        latent_dimension, 
        embedding_dimensions, 
    ...
    def call(self, inputs, training=False, outputs=None):
        #mu = tf.cast(
        #    tf.fill((tf.shape(inputs)[0], self.latent_dimension,), 0.0),
        #    dtype=inputs.dtype
        #)
        #logvar = self.logvar(inputs, training)
        mu_logvar = self.mu_logvar(inputs, training)
        ...
        logvar = tf.cast(
            tf.fill((tf.shape(inputs)[0], self.latent_dimension,), np.log(self.epsilon)),
            dtype=inputs.dtype
        )
        ...
        # Metrics for loss
        logprob = self.log_normal(sample, mu, tf.exp(logvar), 0.0)
        prob = tf.exp(logprob)
    ...
    def entropy(self, inputs, inputs_x, mu_x, var_x, training=False, eps=0.0, axis=-1):
        ...
        mu, logvar = tf.split(mu_logvar, 2, axis=-1)
        var = tf.cast(
            tf.fill((tf.shape(inputs)[0], self.latent_dimension,), self.epsilon),
            dtype=inputs.dtype
        )
        ...
        entropy = (self.log_normal(inputs_x, mu, var, 0.0) 
