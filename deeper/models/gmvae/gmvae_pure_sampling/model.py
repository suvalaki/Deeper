...
class Gmvae():
    ...
    def call(self, inputs, training=False):
        ...
        for j in range(self.mc_sam):
            ...
            (
                ...
                px_g_zy__logprob[j], 
                px_g_zy__prob[j]
            ) = self.graphs_px_g_zy.call(
                qz_g_xy__sample[j], inputs, training)
    ...
    def entropy_fn(self, inputs, training=False):
        ...
        #reconstruction
        recon = tf.add_n([ tf.cast(qy_g_x[:,i],tf.float64) * tf.cast(mc_px_g_zy__logprob,tf.float64) for i in range(self.k) ])
        
        #z_entropy
        z_entropy = tf.add_n([ tf.cast(qy_g_x[:,i], tf.float64) * (
            tf.cast(mc_pz_g_y__logprob, tf.float64) 
            - tf.cast(mc_qz_g_xy__logprob, tf.float64)) for i in range(self.k) ])
