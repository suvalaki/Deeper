...
class PurityCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, *args):
        ...
        print(f"PurityTrain: {pur_train:.3f} PurityTest {pur_test:.3f} AMI Train: {ami_train:.3f} AMI Test: {ami_test:.3f}")
        ...
        for cluster in np.unique(y_train_pred):
            ...
            self._write_cluster_average(epoch, X_cluster, f"ClusterAverages/Category_{int(cluster)}")
